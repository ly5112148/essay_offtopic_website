...
def display(request):
    ...
    result['isOffTopic'] = report.isOffTopic
    ...
    if request.POST:
        ...
        if sign == '2':
            ...
            Report.objects.get_or_create(user_id=user, essay_id=essay, error=result['errors'], chart1=result['chart1'],
                                         chart2=result['chart2'], detail=result['detail'], isOffTopic=boolean_value, score= 'no')
...
def main(request):
    ...
    for i in essay:
        ...
        submit = User_Essay.objects.get(essay_id=i["essay_id"]).isSubmit
        list = due_time.split('-')
        dict = {}
        dict['year'] = list[0]
        dict['month'] = list[1]
